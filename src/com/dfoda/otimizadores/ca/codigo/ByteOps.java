...
public class ByteOps {
    ...
    public static String opName(int opcode) {
        ...
        if (result == null) {
            ByteOps.OPCODE_NAMES[opcode] = result = "unused_" + Hex.u1(opcode);
        }
        return result;
    }
    ...
    static {
        ...
        while (i < len) {
            int info;
            int idx = Character.digit(s.charAt(i), 16) << 4 | Character.digit(s.charAt(i + 1), 16);
            switch (s.charAt(i + 3)) {
                case '-': {
                    info = 1;
                    break;
            ...
            int endAt = s.indexOf(59, i);
            ByteOps.OPCODE_INFO[idx] = info;
            ByteOps.OPCODE_NAMES[idx] = s.substring(i, endAt);
            i = endAt + 1;
        }
    }
}
