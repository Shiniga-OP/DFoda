...
import com.dfoda.otimizadores.ca.cst.ConstantPoolParser;
...
public class DirectClassFile implements ClassFile {
    ...
    private void parse0() {
        if (this.bytes.size() < 10) {
            throw new ErroCtx("severely truncated class file");
        }
        ...
        if (this.strictParse) {
            if (!this.isGoodMagic(this.getMagic0())) {
                throw new ErroCtx("bad class file magic (" + Hex.u4(this.getMagic0()) + ")");
            }
            if (!this.isGoodVersion(this.getMinorVersion0(), this.getMajorVersion0())) {
                throw new ErroCtx("unsupported class file version " + this.getMajorVersion0() + "." + this.getMinorVersion0());
            }
        }
        ...
        if (this.strictParse) {
            ...
            if (!this.filePath.endsWith(".class") || !this.filePath.startsWith(thisClassName) || this.filePath.length() != thisClassName.length() + 6) {
                throw new ErroCtx("class name (" + thisClassName + ") does not match path (" + this.filePath + ")");
            }
        }
        ...
        if (at != this.bytes.size()) {
            throw new ErroCtx("extra bytes at end of class file, at offset " + Hex.u4(at));
        }
        if (this.observer != null) {
