...
public final class Dop {
    ...
    public Dop(int opcode, int family, int nextOpcode, InsnFormat format, boolean hasResult) {
        if(!Opcodes.isValidShape(opcode)) throw new IllegalArgumentException("bogus opcode");
        if(!Opcodes.isValidShape(family)) throw new IllegalArgumentException("bogus family");
        if(!Opcodes.isValidShape(nextOpcode)) throw new IllegalArgumentException("bogus nextOpcode");
